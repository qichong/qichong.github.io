...
const SOURCES = {
  lion: 'code4fukui/vr-cats · kenchoo · CC BY-NC-SA 4.0',
  tiger: '项目原有模型 · 来源记录未变更',
  fox: 'WildMesh 3D / Sketchfab · CC BY 4.0',
  black_panther: '仓库上传资产 · 原始来源待补充',
  lioness: 'WildMesh 3D / Sketchfab · CC BY 4.0',
  alligator: 'WildMesh 3D / Sketchfab · CC BY 4.0',
  shark: 'Open Water / Optic_idealist · CC BY 4.0',
  whale: 'Open Water / Bohdan Lvov · CC BY 4.0',
  horse: 'WildMesh 3D / Sketchfab · CC BY 4.0',
  seagull: 'Open Water / The lighthouse keeper / geminga · CC BY-SA 4.0',
  macaw: 'Open Water / Mateus Schwaab · CC BY 4.0',
  starfish: 'Open Water / Digital Atlas of Ancient Life · CC0 1.0',
  swordfish: 'Open Water / Quaternius · CC0 1.0',
  tuna: 'Open Water / Quaternius · CC0 1.0',
  deer: 'WildMesh 3D / Sketchfab · CC BY 4.0',
  rabbit: 'WildMesh 3D / Sketchfab · CC BY 4.0'
};
...
function playAnimation(index) {
  const item = currentAnimations[index];
  if (!item || !mixer) return;
  stopMixer();
  item.action.reset().fadeIn(0.15).play();
  const once = /dead|death|die|dying|fall|faint|attack|roar|growl|bite|claw|strike|hit/.test(item.name.toLowerCase());
  item.action.setLoop(once ? THREE.LoopOnce : THREE.LoopRepeat, once ? 1 : Infinity);
  item.action.clampWhenFinished = once;
  document.querySelectorAll('.action').forEach((b) => b.classList.toggle('active', b.dataset.actionIndex === String(index)));
  $('#actionHint').textContent = `${SPECIES[active].name} · ${item.name} · ${item.duration.toFixed(2)}s`;
}
...
function renderActions(gltf) {
  ...
  if (mixer) mixer.stopAllAction();
  mixer = clips.length ? new THREE.AnimationMixer(gltf.scene) : null;
  ...
  $('#actions').innerHTML = clips.length
    ? currentAnimations.map((item) => `<button class="action" data-action-index="${item.index}" title="${item.name.replace(/"/g, '&quot;')}">${animationLabel(item.name, item.index)} <small>${item.duration.toFixed(1)}s</small></button>`).join('')
    : '<div class="no-actions">这个 GLB 没有嵌入骨骼动画</div>';

  const source = SOURCES[active] || '来源信息未记录';
  $('#actionHint').textContent = clips.length
    ? `${clips.length} 个实际动画 · 来源：${source}`
    : `0 个动画 · 来源：${source}`;
...
async function load(kind) {
  ...
  const task = (async () => {
    const d = SPECIES[kind];
    setLoading(`正在读取 ${d.name}`, '读取 GitHub Pages 同源本地 GLB，并检查内置动画。');
    ...
    if (signature !== 'glTF') throw new Error(`${d.name} 不是有效 GLB`);
...
$('#actions').addEventListener('click', (e) => {
  ...
  if (!b) return;
  playAnimation(Number(b.dataset.actionIndex));
});
...
(async () => {
  ...
  setLoading('正在准备动物馆', '正在读取首屏动物并检查它们实际包含的动画。');
  const results = await Promise.allSettled(firstThree.map((kind) => load(kind)));
  const first = results.findIndex((x) => x.status === 'fulfilled');
  if (first >= 0) await show(firstThree[first], results[first].value);
  $('#loadingMessage').textContent = '动作按钮会直接按照当前 GLB 内的真实 animation clip 展示。';
  ...
  const worker = async () => {
    while (queue.length) {
      ...
      try { await load(kind); }
      catch (error) { console.warn(`${kind} background preload failed`, error); }
    }
  };
