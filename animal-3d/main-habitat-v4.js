...
const habitatType = (kind) => habitatFor[kind] || 'meadow';

function makeMaterialFix(material) {
  if (!material) return;
...
function prepareModel(model, kind) {
  ...
  model.traverse((obj) => {
    ...
    materials.forEach(makeMaterialFix);
  });
}

function add(group, geometry, material, position, rotation = [0, 0, 0], scale = [1, 1, 1]) {
...
function addAcacia(g, x, z, s = 1) {
  add(g, new THREE.CylinderGeometry(.07*s,.16*s,1.55*s,8), mat(0x624b31), [x,.78*s,z]);
  const crown = mat(0x4f672c);
...
function addTree(g, x, z, s = 1) {
  add(g, new THREE.CylinderGeometry(.065*s,.13*s,1.25*s,7), mat(0x624c35), [x,.63*s,z]);
  add(g, new THREE.SphereGeometry(.55*s,9,7), mat(0x4f733f), [x,1.35*s,z]);
  add(g, new THREE.SphereGeometry(.4*s,9,7), mat(0x638b4d), [x-.28*s,1.48*s,z+.08]);
}

function addPine(g, x, z, s = 1) {
  add(g, new THREE.CylinderGeometry(.055*s,.11*s,1*s,7), mat(0x5c4938), [x,.5*s,z]);
  add(g, new THREE.ConeGeometry(.58*s,1.35*s,9), mat(0x315442), [x,1.2*s,z]);
  add(g, new THREE.ConeGeometry(.43*s,1*s,9), mat(0x3e644d), [x,1.72*s,z]);
}

function addGrass(g, x, z, s = 1, color = 0x709844, count = 6) {
  for (let i=0;i<count;i++) {
    const a = i/count*Math.PI*2;
    const h = (.25 + (i%4)*.07)*s;
    const blade = add(g, new THREE.ConeGeometry(.024*s,h,4), mat(color), [x+Math.cos(a)*.09,h/2,z+Math.sin(a)*.09]);
...
function addFlower(g, x, z, s = 1, c = 0xf1b0c7) {
  add(g, new THREE.CylinderGeometry(.01*s,.015*s,.2*s,5), mat(0x4b7b43), [x,.1*s,z]);
  for (let i=0;i<5;i++) {
    const a = i*Math.PI*2/5;
    add(g, new THREE.SphereGeometry(.042*s,6,5), mat(c), [x+Math.cos(a)*.055*s,.215*s,z+Math.sin(a)*.055*s]);
  }
  add(g, new THREE.SphereGeometry(.022*s,6,5), mat(0xf5d46d), [x,.22*s,z]);
}

function addRock(g, x, z, s = 1, c = 0x6b6b63) {
  add(g, new THREE.DodecahedronGeometry(.3*s,0), mat(c), [x,.18*s,z],[0,(x+z)*.12,0],[1.25,.72,1]);
}

function addMountain(g, x, z, s = 1) {
  add(g, new THREE.ConeGeometry(1.65*s,2.9*s,6), mat(0x68736f), [x,1.45*s,z]);
  add(g, new THREE.ConeGeometry(.62*s,1.2*s,6), mat(0xe8eef1), [x,2.3*s,z-.02]);
}
...
function buildEnvironment(kind) {
  clearEnvironment();
  const type = habitatType(kind);
  const cfg = HABITATS[type];
  scene.background = new THREE.Color(cfg.sky);
  scene.fog = new THREE.FogExp2(cfg.fog, cfg.density);

  const hemi = new THREE.HemisphereLight(type === 'savanna' ? 0xffe6c8 : 0xe8f5ff, type === 'savanna' ? 0x60472b : 0x304238, type === 'savanna' ? 2.25 : 2.15);
  ...
  const rim = new THREE.DirectionalLight(type === 'savanna' ? 0x9a6c3f : 0x7ba0b4, 1.15);
  rim.position.set(-6,4,-7);
  lights = [hemi, sun, rim];
  scene.add(...lights);

  environment = new THREE.Group();
  add(environment, new THREE.CircleGeometry(14,96), mat(cfg.ground), [0,-.02,0], [-Math.PI/2,0,0]);

  if (type === 'savanna') {
    add(environment, new THREE.SphereGeometry(.72,24,16), mat(0xffd17a,{emissive:0x8e481a,emissiveIntensity:1}), [4.6,4,-8]);
    [[-8,-8,1.7],[-4.8,-7.5,1.2],[4.6,-8.3,1.4],[7.8,-5.8,1],[-6,3,1],[6,2,1.1]].forEach(v=>addAcacia(environment,...v));
    for(let i=0;i<24;i++) addGrass(environment,-8+(i*1.91)%16,-6+(i*2.47)%12,1+(i%3)*.2,0x8c9840,7);
    ...
    const cs=[0xe8a7c2,0xf0c77d,0xc9b2e9,0xf5ebbb];
    for(let i=0;i<55;i++) addFlower(environment,-7.5+(i*1.73)%15,-5.8+(i*2.14)%11,.8+(i%3)*.16,cs[i%4]);
    ...
    add(environment,new THREE.CircleGeometry(6.2,64),mat(0x3f7f87,{transparent:true,opacity:.72,roughness:.12}),[0,.012,-1.1],[-Math.PI/2,0,0]);
    ...
    add(environment,new THREE.CircleGeometry(6.8,64),mat(0x4b9fba,{transparent:true,opacity:.76,roughness:.16}),[0,.015,-2],[-Math.PI/2,0,0]);
    for(let i=0;i<20;i++) addRock(environment,-7+(i*1.73)%14,-5+(i*2.19)%10,.4+(i%3)*.15,0x827a68);
  } else if (type === 'ocean') {
    add(environment,new THREE.CircleGeometry(14,96),mat(0x2d7c9b,{transparent:true,opacity:.88,roughness:.15}),[0,-.08,0],[-Math.PI/2,0,0]);
    for(let i=0;i<18;i++) addRock(environment,-8+(i*1.6)%16,-7+(i*2)%14,.45+(i%3)*.16,0x497d86);
  } else {
    add(environment,new THREE.CircleGeometry(14,96),mat(0x337487),[0,-.1,0],[-Math.PI/2,0,0]);
...
function buildActions(gltf) {
  ...
  for (const clip of gltf.animations) {
    const n = clip.name || '';
    if (!actions.idle && /idle|stand|survey|rest|breath|wait/i.test(n)) actions.idle = mixer.clipAction(clip);
    if (!actions.walk && /walk|walking|stroll|run|running|locomotion|move/i.test(n)) actions.walk = mixer.clipAction(clip);
    if (!actions.roar && /roar|growl|attack|call|cry|bite/i.test(n)) actions.roar = mixer.clipAction(clip);
    if (!actions.dead && /dead|death|die/i.test(n)) actions.dead = mixer.clipAction(clip);
  }
...
function playAction(mode) {
  ...
  if (!mixer) { $('#actionHint').textContent='当前模型没有骨骼动作。'; return; }
...
function showModel(kind, gltf) {
  ...
  if (current && mixer) mixer.stopAllAction();
...
async function selectAnimal(kind) {
  ...
  renderButtons();
  try {
    $('#modelStatus').textContent = `${SPECIES[kind].name} · 加载中`;
  ...
  if (environment) {
    environment.traverse((o)=>{
      if (o.isMesh && o.userData.grass) {
        o.rotation.z = Math.sin(performance.now()*.0015 + (o.userData.phase||0) + o.position.x + o.position.z)*.05;
      }
    });
  }
  controls?.update();
...
$('.animals').addEventListener('click',(e)=>{ const b=e.target.closest('.animal'); if(b) selectAnimal(b.dataset.animal); });
$('#actions').addEventListener('click',(e)=>{ const b=e.target.closest('.action'); if(b) playAction(b.dataset.action); });
$('#voiceBtn').addEventListener('click',()=>{ const d=SPECIES[active]; speechSynthesis.cancel(); const u=new SpeechSynthesisUtterance(d.speech||`现在看到的是${d.name}。`); u.lang='zh-CN'; u.rate=.92; speechSynthesis.speak(u); });
...
(async()=>{
  try {
    ...
    $('#loadingMessage').textContent='正在加载首屏动物模型。';
    const first = ANIMALS.slice(0,3).map((x)=>x.id);
    const results = await Promise.allSettled(first.map(loadModel));
    const ready = results.findIndex((r)=>r.status==='fulfilled');
    if (ready >= 0) showModel(first[ready], results[ready].value);
    setTimeout(()=>$('#loading').classList.add('hide'),350);
    const queue = ANIMALS.map((x)=>x.id).filter((id)=>!first.includes(id));
    for (const kind of queue) { try { await loadModel(kind); } catch (e) { console.warn(`${kind} preload failed`,e); } }
  } catch (e) {
    console.error('animal startup failed',e);
    $('#loading').classList.add('hide');
  }
})();
